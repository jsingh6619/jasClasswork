...
//		drawSlot(pic, 4, 6);
	 ...
	 *  |_|
	 * 
	 * The '*' marks the coordinates of the box(it is not drawn) Avoid an AIOOBE, but any portion of the B
	 * @param pic
	 * @param i == row
	 * @param j == column
	 */
	...
	private static void drawAt(String string, String[][] pic, int row, int col) {
		//always check the row before the column because the row must exist before a column can exist in it
	 ...
	 * 	| |
	 * The coordinates of the slot are where the space between the two vertical lines is found. In other words, there is a vertical line to the left of (i, j) and a vertical line to the right of (i, j)
	...
	/**
	 * write a method that prints every string in the pic, not seperated by arrays and no brackets
	 * @param pic
	 */
